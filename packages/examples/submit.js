...
function progress (data, myContext) {
  print.print({ Progress: `${myContext} ${data}` });
  return false;
}
function onCompletion (err, status, myContext) {
  if (err) {
    ...
    print.print({ onCompletion: `${myContext} ${status.data}` });

    let jobList = store.submitStatus();
    print.printObj(jobList, "List of completed jobs");
    let r = store.submitStatus(myContext);
    store
      .apiCall(r.job.links("log"))
      .then(f => viewer(f))
