...
function initStore(iconfig) {
  var config = iconfig == null ? {} : iconfig;
  var store = (0, _configureSagaStore.default)(config);
  return {
    ...
    getService: _routeToObj.default.bind(null, store),
...
function decodeJWT(store) {
  var token = loggedOn(store)['token'];
  return token === null ? {} : (0, _jwtDecode.default)(token);
}
