...
var errColors = require('./errColors');

var stdColors = require('./stdColors');

var titleColors = require('./titleColors');

var prettyjson = require('prettyjson');

function printErr(msg) {
  var msg1 = (0, _typeof2["default"])(msg) === 'object' ? msg : {
    Error: msg
  };
...
function view(f, title) {
  if (f.type === 'itemsList') {
    ...
    printTitle({
      Type: "".concat(f.type, "  ResultType: ").concat(f.resultType)
    });
...
function viewList(type, itemsList, title) {
  printTitle({
    title: "".concat(title, "  type: ").concat(type, "  size: ").concat(itemsList.size)
  });
...
function viewLinks(type, itemsList, title) {
  ...
  itemsList.forEach(function (l, k) {
    print({
      rel: k
    });
  });
...
function printObj(d, title) {
  var d1 = Immutable.Iterable.isIterable(d) ? d.toJS() : (0, _typeof2["default"])(d) === 'object' ? d : {
    value: d
  };
  print(d1, title);
}
