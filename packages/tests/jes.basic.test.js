...
async function run() {
  ...
  debugger;
  let jesSummary = await restaflib.jesRun(store, "mcpjobdef", { a: 1, b: 2, c: 'xxx' });
  console.log(jesSummary);
  console.log(jesSummary.log);
  console.log(jesSummary.listing);
  /*
  let r = await getContent(store, jesSummary.files['log.txt']);
  console.log('log.txt', r);

  r = await getContent(store, jesSummary.files['listing.txt']);
  console.log('listing.txt', r);
  */
