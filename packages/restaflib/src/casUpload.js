...
async function casUpload (store, session, source, output,save){

    // parse source file to fgure out what we are uploading
    let fileInfo = uploadSetup(source, output);
    // read the file
    let inputData = await fs.readFile(source, fileInfo.fileOptions);

    // transfrom data if necessary for upload
    let buf = fileInfo.transform(inputData, fileInfo);
